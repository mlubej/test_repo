...
\subsection{Templates in signal fits}\label{sec:templates-in-signal-fits}
...
\end{itemize}
As mentioned in Chapter \ref{sec:background-suppression}, the majority of the background comes from $B \bar B$ events. Various processes contribute to this background which are well known and measured, so we make use of these measurements to fix these contributions in MC fits and appropriately constrain them when fitting to real data. The remaining $B \bar B$ background is merged into a single template.

When fitting the signal sample, the shape of all templates is fixed and, except in the case of constrained background templates, the template yields were left as floating parameters in the fit.

\subsection{Templates in control fits}

In case of control region fit the background composition is different, as seen in Figure \ref{fig:cs_bkg_after}. Due to the strict cut of the $m_{KK}$ around the $D^0$ mass window, most of the decays with a $D^0$ proceed via $D^0 \to K^+K^-$ decay. In this case, the following fit templates are chosen
\begin{itemize}
	\item signal template,
	\item $q \bar q$ template,
	\item $C_0:~B^+ \to \bar{D} {}^0 \ell^+ \nu,~D^0 \to K^-K^+$ (control decay),
	\item $C_1:~B \to \bar{D} {}^* \ell^+ \nu,~D^0 \to K^-K^+$ template,
	\item other $B \bar B$ BKG template.

\end{itemize}
As in the previous case, the shape of templates is fixed. The yield of all template, except the $B \to \bar D {}^* \ell^+ \nu,~D^0 \to K^-K^+$ template, are floated. Since the $B \to \bar D {}^* \ell^+ \nu,~D^0 \to K^-K^+$ decay is well known and measured, we take use of this fact in the form of a ratio $N_{C_1}/N_{C_0}$, which is fixed to the MC value in case of the MC fit and constrained to the measured value in case of fits to real data.
...
In this section the procedure for fixing or constraining the physics parameters in the fits to MC or data.

\subsection{Signal Fit Constraints}
As mentioned in subsection \ref{sec:templates-in-signal-fits}, some parts of the $B \bar B$ background is well known and can be constrained based on world measurements. The channels are constrained based on the following formula

X

\subsection{Control Fit Constraints}
In order to define constraints in control fits, the channels defined in Table X are used.
\begin{table}[H]
	\centering
	\begin{tabular}{|l|l|l|c|c|c|l|}
		\hline
		Category & Channel & Decay mode & $N_{MC}$ & $\mathcal{B}_{MC}$ & $\mathcal{B}_{PDG}$ & Ref. \\
		\hline
		\multirow{2}{*}{$C_0$} & \multirow{2}{*}{$N_{00}$} & $B^+ \to \bar D {}^{0} \ell^+ \nu$ & \multirow{2}{*}{X} & X & X & [X]\\
		 & & $D^0 \to K^-K^+$ &  & X & X & [X]\\
		\hline
		\multirow{4}{*}{$C_1$} & \multirow{2}{*}{$N_{10}$} & $B^+ \to \bar D {}^{*0} \ell^+ \nu$ & \multirow{2}{*}{X} & X & X & [X]\\
		 & & $D^0 \to K^-K^+$ &  & X & X & [X]\\	
		 \cline{2-6}	 
		 & \multirow{2}{*}{$N_{11}$} & $B^0 \to D {}^{*-} \ell^+ \nu$ & \multirow{2}{*}{X} & X & X & [X]\\		 
		 & & $D^0 \to K^-K^+$ &  & X & X & [X]\\
		 %		\multirow{2}{*}{1}&X\\
%		&X\\
		\hline
...
\begin{equation}
r_i = \frac{N_{C_i}^{MC}\times \rho_i}{N_{C_0}^{MC}\times \rho_0}
\end{equation}
