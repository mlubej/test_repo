...
At this point of the analysis, we do not apply any intelligent cuts yet, which results in a large number of available particles and their combinations. In order to minimize this effect, we perform this part of the study on a smaller subset of the available generic MC, experiment no. $23$ and $31$.
...
In this section we use the pre-selected charged particles to make particle combinations, which correspond to $B$ meson candidates. When a $B$ meson candidate is selected, additional features can be calculated and used for $\mathrm{FOM}$ optimization. Since we are still performing this part of the study on a smaller subset of the full available MC sample, we will perform under-optimized cuts in order to optimize them later on the full Belle MC sample.
...
\caption{Normalized vertex fit probability distribution for signal and background $B$ meson candidates in logarithmic scale(left) and $\mathrm{FOM}$ optimization of the vertex fit probability (right) for the subset of the full Belle MC sample, and FOM optimization on the full Belle MC sample (bottom).}
\label{fig:vtx}
\end{figure}

The vertex fit probability cut was optimized on a smaller subset of the available MC, and later on the full Belle MC. The corresponding pre-cut and optimized cut are
\begin{itemize}
\item Pre-cut: $P(\chi^2,NDF) > 1.0\E{-3}$,
...
For the correctly reconstructed candidates, this variable lies in the $[-1,1]$ region, though only to a certain precision, due to the finite detector resolution. Background candidates, however, populate also the non-physical regions, as shown in Figure \ref{fig:cosby} (left). We impose an under-optimized cut on this variable from Figure \ref{fig:cosby} (right) to discard a large amount of background on the subset of the Belle MC, and later on the full Belle MC sample (bottom).
\begin{itemize}
\item Pre-cut: $\vert \cos \left(\theta_{BY}\right) \vert < 1.20$,
\item Optimal cut: $\vert \cos \left(\theta_{BY}\right) \vert < 1.05$.
\end{itemize}
...
\includegraphics[width=\linewidth]{fig/cosBY_precise}
\caption{Normalized $\cos \theta_{BY}$ distribution for signal and background $B$ meson candidates (left) and $\mathrm{FOM}$ optimization of the $\cos \theta_{BY}$ variable (right) for the subset of the Belle MC, and optimization of the FOM on the full Belle MC sample (bottom).}
\label{fig:cosby}
\end{figure}
