...
The optimal PID cuts for kaons are
\begin{itemize}
	\item $K/\pi > 0.6$,
	\item $K/p > 0.1$.
\end{itemize}
...
Figure \ref{fig:mbc_de_pre} shows the distributions of $\Delta E$ (left) and $M_{BC}$ (right) for signal and major types of background after the pre-cuts. Both signal components (all signal and perfect signal) are scaled up with respect to the background components but are in proper scale one to another. The effects of missing particles are clearly seen based on the shape difference between all and perfect signal.
...
\begin{figure}[H]
	...
	\caption{$\mathrm{FOM}$ optimization of the $m_{miss}^2$ cut in the signal region.}
	\label{fig:missm2opt}
\end{figure}
...
\label{s:ss}
In this section, one can find the full summary of final selection cuts in the event reconstruction, from FSP particles up to the $B$ meson.
