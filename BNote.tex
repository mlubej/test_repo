...
<<<<<<< HEAD
We use the converted $K_S^0$ candidates from the existing Belle particle list and use an existing Neural Network classifier result with \textit{nisKsFinder} from [] in order to select only the good $K_S^0$ candidates. Figure X shows the distribution of the $K_S^0$ invariant mass for signal and background candidates, before and after the classifier cut.
=======
...
>>>>>>> 74e487472f20575f376125309b5598dd3d401791
...
\subsubsection{Defining duplicate track pairs}

In this step we wish to find a handle on secondary tracks from low momentum curlers and decays in flight. The main property of such tracks is the angle between them, which is very close to $0^\circ$, since tracks deviate only slightly from the initial direction, or $180^\circ$, since they can also be reconstructed in the opposite way. Figure X shows the distribution of the angle between two track pairs for random tracks and duplicate tracks, where the latter were reconstructed as two same-sign or opposite-sign tracks.
=======
...
<<<<<<< HEAD
If the particle decayed mid-flight, or produced multiple tracks due to being a low-momentum curler, then, as the name suggests, these particles most likely had low transverse momentum component $p_T$. Since both tracks originate from the same initial particle, the momentum difference should also peak at small values. Figure X shows the momentum and momentum difference of tracks which belong to a random or a duplicate track pair.
=======
>>>>>>> 74e487472f20575f376125309b5598dd3d401791
...
In this section the results of the ROE clean-up are shown. It is clear that the clean-up affects the shape of various distribution in the sense that it increases resolution. However, since the reconstruction procedure, which is thoroughly described in chapter X, also uses cuts on variables, which are affected by the clean-up, the ROE clea-up also increases the efficiency of the reconstructed sample. 
...
It can also be seen that the determination of the masses of charged FSP particles in Eq. (\ref{eq:pcharged}) based on PID is quite successfull, since the benefits of using true masses from Monte Carlo are very small.

Another variable which heavily depends on the clean-up is the charge product of the reconstructed $B$ meson and the charge of the ROE, which should for charged $B$ meson pairs be equal to 
\begin{equation}
Q_{B^+} Q_{B^-} = -1,
\end{equation}

but is in fact a distribution around this value due to imperfect event reconstructions, as shown in Figure X. As a cross-check, we can also look at $\Delta E$ and $M_{BC}$ variables for each value of the charge product. These plots are shown in Figure X and they show a clear increase in efficiency and resolution for the correct value of the charge product, with a slight efficiency increase also for other values.

PLOT

\subsection{Comparison to Belle clean-up}
