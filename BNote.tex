...
The classifier is able to distinguish between random and duplicate track pairs in a very efficient manner, as shown in Figure X. The $FOM$ function for optimal cut selection is shown in Figure X (right), along with the angle between the two tracks after the optimal cut (left). The optimal cut for duplicate track selection is
\begin{itemize}
