\documentclass[headings=standardclasses,headings=big,oneside,a4paper,openany,12pt]{scrbook}

\newcommand {\e}[1]{\mathrm{~#1}}
\newcommand {\E}[1]{\times 10^{#1}}
\newcommand {\vars}{$\Delta E$ and $M_{BC}$}
\newcommand {\varss}{$\Delta E$ in $M_{BC}$}
\newcommand {\btbii}{\texttt{B2BII}}
\newcommand {\decaya}{$B \to K K \ell \nu$}
\newcommand {\decayb}{$B^+ \to K^+ K^- \ell^+ \nu$}

\usepackage{fancyhdr}

\usepackage[slovene,english]{babel}
\usepackage{csquotes}
\usepackage{paralist}
\usepackage{caption}
\usepackage{cancel}
\usepackage{longtable}
\usepackage{multirow}
\usepackage{titling}
\usepackage{amsmath,amssymb,amsfonts,nicefrac}
\usepackage{graphicx}
\usepackage{color}
\usepackage{float}
\usepackage{mathtools}
\usepackage[unicode,pdftex,colorlinks=true,citecolor=blue,linkcolor=black,urlcolor=blue,bookmarks=true]{hyperref}
\usepackage{dictsym}
\usepackage{braket}
\usepackage{slashed}
\DeclareMathOperator{\arcsinh}{arcsinh}
\usepackage{enumerate}
\usepackage{array}
\usepackage{tikz}
\usepackage{subfigure}
\usepackage{bm}
\usepackage[a4paper,inner=3.5cm,outer=2.5cm,top=2.5cm,bottom=2.5cm,pdftex]{geometry}
\usepackage[titletoc,title]{appendix}
\usepackage{icomma}
\usepackage{booktabs}
\usepackage{lineno}
%\linenumbers


\usepackage[minnames=2,maxnames=3,style=numeric,backend=bibtex,sorting=none]{biblatex}
\addbibresource{bib/mybib}

\usetikzlibrary{fadings}
\raggedbottom
\pdfsuppresswarningpagegroup=1

% supress longtable bug warning
\makeatletter
\def\LT@start{%
	\let\LT@start\endgraf
	\endgraf\penalty\z@\vskip\LTpre
	\dimen@\pagetotal
	\advance\dimen@ \ht\ifvoid\LT@firsthead\LT@head\else\LT@firsthead\fi
	\advance\dimen@ \dp\ifvoid\LT@firsthead\LT@head\else\LT@firsthead\fi
	\advance\dimen@ \ht\LT@foot
	\edef\restore@vbadness{\vbadness\the\vbadness\relax}% (added)
	\vbadness=\@M % (added)
	\dimen@ii\vfuzz
	\vfuzz\maxdimen
	\setbox\tw@\copy\z@
	\setbox\tw@\vsplit\tw@ to \ht\@arstrutbox
	\setbox\tw@\vbox{\unvbox\tw@}%
	\vfuzz\dimen@ii
	\restore@vbadness % (added)
	\advance\dimen@ \ht
	\ifdim\ht\@arstrutbox>\ht\tw@\@arstrutbox\else\tw@\fi
	\advance\dimen@\dp
	\ifdim\dp\@arstrutbox>\dp\tw@\@arstrutbox\else\tw@\fi
	\advance\dimen@ -\pagegoal
	\ifdim \dimen@>\z@\vfil\break\fi
	\global\@colroom\@colht
	\ifvoid\LT@foot\else
	\advance\vsize-\ht\LT@foot
	\global\advance\@colroom-\ht\LT@foot
	\dimen@\pagegoal\advance\dimen@-\ht\LT@foot\pagegoal\dimen@
	\maxdepth\z@
	\fi
	\ifvoid\LT@firsthead\copy\LT@head\else\box\LT@firsthead\fi\nobreak
	\output{\LT@output}%
}
\makeatother

% more space at list of figures
\makeatletter
\renewcommand*\l@figure{\@dottedtocline{1}{1em}{3.2em}}
\makeatother

\newcommand{\changelocaltocdepth}[1]{%
	\addtocontents{toc}{\protect\setcounter{tocdepth}{#1}}%
	\setcounter{tocdepth}{#1}%
}


\title{{\Large BELLE NOTE 1486}\\\huge {Measurement of the decay $B^+ \to K^+K^-\ell^+\nu_\ell$ with B2BII}}
\author{Matic Lubej}
\date{Ljubljana, 2018}
%
%
\begin{document}
\begin{titlingpage} %This starts the title page
\phantom{test}
\vspace{5cm}

\begin{center}
\thetitle
\vspace{0.5 cm} \\
{\Large \theauthor}\\
\vfill
{\Large\thedate}
\end{center}
\end{titlingpage}

\pagestyle{plain}
\pagenumbering{roman}

\chapter*{Changelog}

\begin{itemize}
\item 4. May: First submission of the note.
\item 16. May: Added ROE validation section.
\item 7. July: update of whole BNOTE, B2BII chapter, finishing control fits.
\item 19. August:
	\begin{itemize}
		\item More details on the fit process and background splitting.
		\item Adding sources of systematics
		\item Ready for box opening
	\end{itemize}
\item 5. October:
\begin{itemize}
	\item Added fits to data
	\item Added all sources of systematics
	\item New model for uncertainty study
	\item $q^2$ calculation description
	\item efficiency as a function of $m_{KK}$ and $q^2$
	\item Fit yields in bins of $m_{KK}$ and $q^2$
\end{itemize}
\end{itemize}

\tableofcontents
\addtocontents{toc}{~\hfill\textbf{Page}\par}

\include{chapters/1_intro}
\include{chapters/2_data}
\include{chapters/3_b2bii}
\include{chapters/4_evt}
\include{chapters/5_roe}
\include{chapters/6_bkg}
\include{chapters/7_fit}
\include{chapters/8_sys}

\printbibliography

\appendix
\changelocaltocdepth{1}
\include{chapters/A_roe}
\include{chapters/B_mva}
\include{chapters/C_other}


\end{document}


