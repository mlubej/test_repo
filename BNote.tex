...
\begin{itemize}
\item Fit region: $M_{BC} \in [5.1,\,5.295]\e{GeV}/c^2$ and $\Delta E \in [-1.0,\,1.3]\e{GeV}$,
...
This chapter shows the procedure in suppressing various kinds of background by applying a combination of rectangular cuts and cuts on MVA classifier output.

\section{Event categorization}

The missing information due to an escaping neutrino in our reconstructed channel is replaced by information from the companion $B$ meson. Since this is an untagged reconstruction, the quality of the companion $B$ meson matters greatly. Perfect reconstruction of a hadronically decaying companion $B$ meson yields pronounced peaks at $\Delta E \approx 0$, $m_{miss}^2 \approx 0$ and $M_{BC} \approx m_B$, while imperfect reconstruction due to any kind of missing particles results in long tails and/or a shift from the desired value in the mentioned distributions. These effects are undesired, since they make it harder to separate signal from background.
...
however, this value is distributed due to possible missing charged particles in the reconstruction. Secondly, we train an MVA classifier based on ROE object properties, in order to recognize companion $B$ mesons, which decayed hadronically. The details of the training are described in subsection \ref{subs:HDMVA}.

We define the 4 categories in the following way
%TODO check
\begin{enumerate} 
\item[I)] $q_{B^\pm}~q_{B^\mp} = -1$ and $BDT_{had.} > X$,
\item[II)] $q_{B^\pm}~q_{B^\mp} != -1$ and $BDT_{had.} > X$,
\item[III)] $q_{B^\pm}~q_{B^\mp} = -1$ and $BDT_{had.} < X$,
\item[IV)] $q_{B^\pm}~q_{B^\mp} != -1$ and $BDT_{had.} < X$.
\end{enumerate}

Different categories for all signal candidates after the pre-cuts are shown in Figure X. Category I has the best resolution, so we use it in our analysis from this point on.
